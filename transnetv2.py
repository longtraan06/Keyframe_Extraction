...
class TransNetV2:
    ...
    def predict_video(self, video_fn: str, save_frames_dir: str = None):
        try:
            import ffmpeg
        ...
        print(f"[TransNetV2] Extracting original-size frames from {video_fn}")
        ...
        # ➤ Extract raw RGB frames với size gốc
        ...
        # ✅ Nếu yêu cầu lưu từng frame
        if save_frames_dir:
            os.makedirs(save_frames_dir, exist_ok=True)
            from PIL import Image
            for i, frame in enumerate(video):
                frame_path = os.path.join(save_frames_dir, f"frame_{i:05d}.png")
                Image.fromarray(frame).save(frame_path)

        # ➤ Resize video về kích thước (27, 48) để feed vào TransNetV2
        import cv2
        resized_video = np.array([cv2.resize(f, (48, 27)) for f in video])

        return (resized_video, *self.predict_frames(resized_video))
...
def main():
    ...
    parser.add_argument('--save-frames-dir', type=str, default=None,
                    help="Directory to save extracted frames (optional)")
    ...
    for file in args.files:
        ...
        video_frames, single_frame_predictions, all_frame_predictions = \
            model.predict_video(file, save_frames_dir=os.path.join("frames", os.path.basename(file)))
...
if __name__ == "__main__":
    main()
#python3 transnetv2.py /home/liex/Desktop/Work/keyframe_extraction/L01_V001.mp4 [--visualize]
